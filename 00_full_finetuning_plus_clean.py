...
#sys.path.append("/datadrive1/ayush/home/reasoning/synthetic_questions_verifier_finetune/src")

from tasks import math  # Assuming you have this for formatting functions
...
class ScriptArguments:
    ...
    train_dataset_path: Optional[str] = field(
        default="/datadrive1/ayush/home/reasoning/synthetic_questions_verifier_finetune/datasets/verifier_outputs/DeepSeek-R1-Qwen-1.5_dedup_fil_0to34000_thresh75_30749points.jsonl",
    ...
    output_dir: Optional[str] = field(
        default="/datadrive1/ayush/home/myc_new5/ayush/reasoning/synq-ver/ckpts/colm/models_tmp_verf_syn_SFTrainer_fullft_20epochs",
    ...
    lr_scheduler_type: Optional[str] = field(
        default="linear", 
        metadata={"help": "Type of learning rate scheduler. Options include linear, cosine, cosine_with_restarts, polynomial, constant, constant_with_warmup"}
    )
...
# ----------------------- ADDED: continuous optimizer cleanup -----------------------
from transformers import TrainerCallback
import glob

class OptimizerCleanupCallback(TrainerCallback):
    """
    After each checkpoint save, remove optimizer.pt from all but the latest checkpoint.
    Runs only on the main process to avoid race conditions.
    """
    def __init__(self, checkpoints_root: str):
    ...
    def on_save(self, args, state, control, **kwargs):
        ...
        try:
            checkpoints = sorted(
                [p for p in glob.glob(os.path.join(self.checkpoints_root, "checkpoint-*")) if os.path.isdir(p)],
            ...
            for ckpt in checkpoints[:-1]:
                ...
                if os.path.exists(opt_path):
                    # Delete optimizer from older checkpoints to save space
                    print(f"[OptimizerCleanup] Deleting {opt_path}")
                    try:
                        os.remove(opt_path)
                    except Exception as e:
                        print(f"[OptimizerCleanup] Failed to delete {opt_path}: {e}")
            print(f"[OptimizerCleanup] Kept optimizer.pt only in latest: {latest}")
        except Exception as e:
            print(f"[OptimizerCleanup] Skipped due to error: {e}")
        return control

# Register the callback so it triggers after every save
trainer.add_callback(OptimizerCleanupCallback(checkpoints_root=script_args.output_dir))
# -----------------------------------------------------------------------------------

# Start training
print("Starting training...")
#trainer.train()
resume_ckpt = script_args.load_from_checkpoint if script_args.load_from_checkpoint else None
